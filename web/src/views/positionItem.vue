...
<div id="home">
    ...
    <div class="body">
        <div class="main">
            <div class="mainBox">
                <div class="mainTitle">旅游景点详情页</div>
                <div class="box">
                    <span class="name">【{{item.title}}】 {{ item.type }}</span>
                    <div class="labelBox">
                        <el-button type="warning" :disabled="token===undefined" @click="addLookFx(item)">{{collectionStatus > 0 ? "取消收藏" : "收藏"}}</el-button>

                        <!-- <el-button type="warning" :disabled="item.canOrder" @click="addOrderFx(item)">预定</el-button>
                        <el-button type="info" :disabled="item.canOrder" @click="toBuyPage">购买</el-button> -->
                        <el-button type="success" @click="toCarListPage">返回</el-button>
                        <el-button type="success" @click="toOrderFx(item.url)">预定</el-button>
                    </div>
                    <div class="labelBox">
                    ...
                    <div style="display:inline">
                        <img :src="item.image" style="width:88%" />
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        addOrderFx(e) {
            var that = this;
            ElMessageBox.prompt('在下面选填备注，支付定金后一周不付款将取消预定资格', '请输入您的预定的定金', {
    ...
    mounted() {
        ...
        if (this.token != undefined && this.token != "") {
            getCollectionStatus({
                ...
                if (res.data.success) {
                    that.collectionStatus = res.data.result;
                }
            })
        }
        this.init();
