...
<div id="home">
    ...
    <header>
        ...
        <!-- <div class="search">
            <input class="searchInput" id="zwzSearchPositionInput" type="text" placeholder="搜索旅游景点">
            <button class="searchBt" @click="selectPositionByTitleFx">搜索</button>
        </div>
        <div :class="class1">
            <input class="searchInput" type="text" placeholder="搜索旅游景点订单" style="boxShadow:none;borderColor:#EFF1F1;">
    ...
    <div class="body">
        <div class="main">
            <div class="mainBox">
                <div class="mainTitle">我的旅游景点预订单 （{{orderList.length}}）</div>
                <div class="box" v-for="(item,index) in orderList" :key="index" @click="toOrderItemPage(item)">
                    ...
                    <div class="labelBox">
                        ...
                        <!-- <div class="smallShu"></div> -->
                        <!-- <el-button type="success" @click="deleteOrder(item)">删除</el-button> -->
                    </div>
                    <div class="labelBox">
                        旅游景点品牌: {{item.brand}}
                    </div>
            ...
            <div class="mainBox">
                <div class="mainTitle">我的旅游景点预约单 （{{orderList2.length}}）</div>
                <div class="box" v-for="(item,index) in orderList2" :key="index" @click="toLookItemPage(item)">
                    <span class="name">【{{item.brand}}】{{item.sellerMoney}}万</span>
                    <div class="labelBox">
                        <span>卖家：{{item.sellerName}}</span>
                        <div class="smallShu"></div>
                        ...
                        <span>买家{{item.buyName}}</span>
                        <div class="smallShu"></div>
                        <!-- <el-button type="success" @click="addLookFx(item)">支付</el-button> -->
                    </div>
                    <div class="labelBox">
                        旅游景点品牌: {{item.brand}}
                    </div>
...
export default {
    ...
    methods: {
        ...
        getByAllOnUserFx2() {
            var that = this;
            getByAllOnUser2().then(res => {
                if (res.data.success) {
                    that.orderList2 = res.data.result;
                } else {
                    ElMessage({
                        type: 'error',
                        message: '查询我的订单失败  ' + res.data.message,
        ...
        toOrderItemPage(e) {
            ...
            this.$router.push("/myOrderItem");
        },
        toLookItemPage(e) {
            Cookies.set("lookId", e.id);
            this.setStore("lookId", e.id);
            this.$router.push("/myLookItem");
        },
