...
<div id="home">
    <div id="header" class="header">
        ...
        <div class="rightBox">
            ...
            <div class="shu"></div>
            <div class="white current">景点排名</div>
            <div class="shu"></div>
            <div class="white">
    ...
    <div class="body">
        ...
        <div class="main">
            <div class="mainBox">
                <div class="mainTitle">海南景点收藏量TOP 10 </div>
                
                <el-row :gutter="20" v-for="(item,index) in newList" :key="index" @click="toCarsItemPage(item)" style="width: 100%;margin-bottom: 10px;">
                    <div class="spotCollPosDiv">收藏 TOP {{ index + 1 }}</div>
                    <el-col :span="24">
                        <el-row :gutter="20">
                            <el-col :span="6">
                            ...
                            <el-col :span="18">
                                <el-row :gutter="20">
                                    ...
                                    <el-col :span="12">
                                        ...
                                        <el-row :gutter="20" class="spanContext">
                                            <span class="spanContextLabel"> 创建时间：</span>{{item.createTime}}
                                        </el-row>
                                        <el-row :gutter="20" class="spanContext">
                ...
                <!-- <div class="box" v-for="(item,index) in newList" :key="index" @click="toCarsItemPage(item)">
                    <span class="name">TOP{{index + 1}}：【{{item.title}}】 {{item.type}}</span>
                    <div class="labelBox">
                    </div>
                    <div class="labelBox">
            ...
            <div class="mainBox">
                <div class="mainTitle">海南景点评分TOP 10 </div>
                <el-row :gutter="20" v-for="(item,index) in newList2" :key="index" @click="toCarsItemPage(item)" style="width: 100%;margin-bottom: 10px;">
                    <div class="spotCollPosDiv" style="color: coral;">评分 TOP {{ index + 1 }}</div>
                    <el-col :span="24">
                        <el-row :gutter="20">
                            <el-col :span="6">
                                <img :src="item.image" style="width:100%" />
                ...
                <!-- <div class="box" v-for="(item,index) in newList2" :key="index" @click="toCarsItemPage(item)">
                    <span class="name">TOP{{index + 1}}：【{{item.title}}】 {{item.type}}</span>
                    <div class="labelBox">
                    </div>
                    <div class="labelBox">
...
export default {
    ...
    methods: {
        init() {
            this.getTopCollectionOnWebFx();
            this.getTopEvaluateOnWebFx();
        },
        toNewsPage() {
            this.$router.push("/news");
        },
        getTopCollectionOnWebFx() {
            var that = this;
            that.newList = [];
            getTopCollectionOnWeb().then(res => {
                console.log(res);
                if (res.data.success) {
        ...
        getTopEvaluateOnWebFx() {
            var that = this;
            that.newList2 = [];
            getTopEvaluateOnWeb().then(res => {
                console.log(res);
                if (res.data.success) {
                    that.newList2 = res.data.result;
                }
            })
        },
        getAllMessageListFx() {
        ...
        addMessageFx() {
            var that = this;
            ElMessageBox.prompt('请输入您的留言', '用户留言', {
                ...
                .then(({
                    ...
                    that.addMessageStr = value;
                    addMessage({
                        str: that.addMessageStr
                    }).then(res => {
                        console.log(res);
                        if (res.data.success) {
                            that.getAllMessageListFx();
