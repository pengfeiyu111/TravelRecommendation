...
function venn(areas, parameters) {
    ...
    var solution = fmin_1.nelderMead(function (values) {
        var current = {};
...
function distanceFromIntersectArea(r1, r2, overlap) {
    ...
    return fmin_1.bisect(function (distance) {
        return circleintersection_1.circleOverlap(r1, r2, distance) - overlap;
    }, 0, r1 + r2);
}
...
function getDistanceMatrices(areas, sets, setids) {
    ...
    var distances = fmin_1.zerosM(sets.length, sets.length), constraints = fmin_1.zerosM(sets.length, sets.length);
...
function constrainedMDSLayout(areas, params) {
    ...
    var norm = fmin_1.norm2(distances.map(fmin_1.norm2)) / distances.length;
    ...
    for (i = 0; i < restarts; ++i) {
        var initial = fmin_1.zeros(distances.length * 2).map(Math.random);
        current = fmin_1.conjugateGradient(obj, initial, params);
    ...
    if (params.history) {
        for (i = 0; i < params.history.length; ++i) {
            fmin_1.scale(params.history[i].x, norm);
        }
    }
    return circles;
}
    ...
    for (var i = 1; i < mostOverlapped.length; ++i) {
        ...
        for (var j = 0; j < overlap.length; ++j) {
            ...
            for (var k = j + 1; k < overlap.length; ++k) {
                ...
                var extraPoints = circleintersection_1.circleCircleIntersection({ x: p1.x, y: p1.y, radius: d1 }, { x: p2.x, y: p2.y, radius: d2 });
...
function lossFunction(sets, overlaps) {
    ...
    for (var i = 0; i < overlaps.length; ++i) {
        ...
        if (area.sets.length == 1) {
            ...
            overlap = circleintersection_1.circleOverlap(left.radius, right.radius, circleintersection_1.distance(left, right));
        }
        else {
            overlap = circleintersection_1.intersectionArea(getCircles(area.sets));
        }
        // eslint-disable-next-line
...
function orientateCircles(circles, orientation, orientationOrder) {
    ...
    if (circles.length == 2) {
        ...
        var dist = circleintersection_1.distance(circles[0], circles[1]);
...
function disjointCluster(circles) {
    ...
    for (var i = 0; i < circles.length; ++i) {
        for (var j = i + 1; j < circles.length; ++j) {
            ...
            if (circleintersection_1.distance(circles[i], circles[j]) + 1e-10 < maxDistance) {
