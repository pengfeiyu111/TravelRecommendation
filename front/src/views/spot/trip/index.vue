            ...
            <Form ref="searchForm" :model="searchForm" inline :label-width="0">
                <Form-item ref="searchForm" :model="searchForm" inline :label-width="0" style="display:flex;">
                    <Form-item label="" prop="title">
                        <Input type="text" v-model="searchForm.title" placeholder="行程名称" clearable style="width: 150px" />
                    </Form-item>
                    <Form-item label="" prop="way">
                        <dict dict="out_way" v-model="searchForm.way" transfer clearable placeholder="交通方式" style="width: 150px" />
                    </Form-item>
                    <Form-item label="" prop="spotId">
                        <Select v-model="searchForm.spotId" placeholder="所属景点" clearable style="width: 150px">
                            <Option v-for="(item,index) in spotList" :key="index" :value="item.id">{{ item.title }}</Option>
                        </Select>
                    </Form-item>
...
export default {
    ...
    methods: {
        ...
        getDataList() {
            this.loading = true;
            getTripList(this.searchForm).then(res => {
                this.loading = false;
                if (res.success) {
